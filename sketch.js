...
var backgroundMusic,waterSound,pirateLaughSound,pirateLaughSound,cannonExplosion ;
var score=0;
...
function preload() {
  ...
  //carregar os arquivos de musica
  ...
  cannonExplosion = loadSound("./assets/assets_cannon_explosion.mp3");
...
//loop do comprimento do boatFrames.
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      if (boats[i]) {
        ...
        boats[i].display();
        boats[i].animate();
        //verificar a colisão entre a torre e o navio
        var collision=Matter.SAT.collides(this.tower,boats[i].body);

        //verificar se o navio tocou a torre e se ele não está quebrado
